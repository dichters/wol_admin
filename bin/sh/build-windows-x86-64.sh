...
# 用法：./bin/sh/build-windows-x86-64.sh [version]
set -euo pipefail
cd "$(dirname "$0")/../.."
...
echo "Building wol-panel v${VERSION} windows/amd64"
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build \
    ...
    -o build/wol-panel.exe .

RELEASE_DIR="release"
mkdir -p "$RELEASE_DIR"

PKG_NAME="wol-panel-${VERSION}-windows-x86-64"
PKG_DIR="${RELEASE_DIR}/${PKG_NAME}"
rm -rf "$PKG_DIR"
mkdir -p "$PKG_DIR"

cp build/wol-panel.exe "${PKG_DIR}/"
